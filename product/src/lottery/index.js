...
let TOTAL_CARDS,
  btns = {
    enterBar: document.querySelector("#enterBar"),
  ...
  document.querySelector("#menu").addEventListener("click", function (e) {
    ...
    switch (target) {
      ...
      case "lottery":
        ...
        //更新剩余抽奖数目的数据显示
        changePrize();
        resetCard().then(res => { startRotateBall().then(res => lottery()); });
        addQipao(`正在抽取[${currentPrize.title}],调整好姿势`);
        switchScreen("lotteryStart");
        break;
      // 重新抽奖
      case "reLottery":
...
function switchScreen(type) {
  switch (type) {
    case "enter":
      btns.enterBar.classList.remove("none");
      btns.lotteryBar.classList.add("none");
      btns.stopBtn.classList.add("none");
      transform(targets.table, 2000);
      break;
    case "lotteryStart":
      btns.stopBtn.classList.remove("none");
      btns.enterBar.classList.add("none");
      btns.lotteryBar.classList.add("none");
      break;
    case "lotteryStop":
...
function createCard(user, isBold, id, showTable) {
  ...
  if (isBold) {
    ...
    element.style.backgroundColor =
      "rgba(0, 127, 127," + (Math.random() * 0.6 + 0.25) + ")";
  }
  //添加公司标识
...
function selectCard(duration = 600) {
  rotate = false;
  let width = 144, length = 192,
    tag = -(currentLuckys.length - 1) / 2,
    locates = [];

  // 计算位置信息, 大于5个分两排显示
  if (selectedCardIndex.length > 5) {
    let xSize = selectedCardIndex.length > 20 ? 10 : 5;
    let yPosition = (selectedCardIndex.length / xSize / 2) * length - length / 2;
    let xOffset = xSize + 1;
    tag = -(xSize - 1) / 2;
    ...
    for (let i = selectedCardIndex.length; i > 0; i--) {
      if (--xOffset <= 0 ) {
        yPosition -= length;
        xOffset = xSize;
        tag = -(Math.min(xOffset, i) - 1) / 2;
      }
      locates.push({
        x: tag * width * Resolution,
        y: yPosition * Resolution
      });
      tag++;
    }
  } else {
  ...
  let zPosition = selectedCardIndex.length > 20 ? 1200 : selectedCardIndex.length > 5 ? 1800 : 2000
...
function startRotateBall() {
  return new Promise((resolve, reject) => {
  scene.rotation.y = 0;
  lotteryTween = new TWEEN.Tween(scene.rotation)
    .to({y: Math.PI * 8},3000)
    .repeat(9999)
    .onUpdate(render)
    .start()
...
function lottery() {
  ...
  if (Lcc) {
    for (let i = 0; i < perCount; i++) {
      let luckyId = randomClient(leftCount);
      currentLuckys.push(basicData.leftUsers.splice(luckyId, 1)[0]); // splice 删除元素， 返回删除元素的数组
      leftCount--;
      leftPrizeCount--;
    
      let cardIndex = randomClient(TOTAL_CARDS);
      while (selectedCardIndex.includes(cardIndex)) {
        cardIndex = randomClient(TOTAL_CARDS);
      }
    ...
    randomServer({
      ...
      count: Math.min(perCount, leftPrizeCount)
    }).then(res => {
      for (let i = 0; i < res.data.length; i++) {
        let currentLucky = basicData.leftUsers.splice(res.data[i], 1)[0];
        if (currentLucky != null) {
          currentLuckys.push(currentLucky); // splice 删除元素， 返回删除元素的数组
          let cardIndex = randomClient(TOTAL_CARDS);
          while (selectedCardIndex.includes(cardIndex)) {
            cardIndex = randomClient(TOTAL_CARDS);
          }
          ...
          setPrizeData(currentPrizeIndex, luckyData ? luckyData.length : 0);
        }
      }
      selectCard();
    }).catch(() => {
      addQipao("网络发生异常， 请重新抽取！");
      setLotteryStatus();
      setPrizeData(currentPrizeIndex, luckyData ? luckyData.length : 0);
    })
  }
...
function shine(cardIndex, color) {
  ...
  card.style.backgroundColor =
    color || "rgba(0, 127, 127," + (Math.random() * 0.6 + 0.25) + ")";
}
...
function shineCard() {
  let maxCard = 10,
    maxUser;
  let shineCard = 10 + randomClient(maxCard);
  ...
  setInterval(() => {
    ...
    for (let i = 0; i < shineCard; i++) {
      let index = randomClient(maxUser),
        cardIndex = randomClient(TOTAL_CARDS);
      // 当前显示的已抽中名单不进行随机切换
 ...
 * 随机抽奖
 */
 function randomClient(num) {
  // Math.floor取到0-num-1之间数字的概率是相等的
  return Math.floor(Math.random() * num);
}

function randomServer(data) {
  return new Promise((resolve, reject) => {
    window.AJAX({
      url: "/lottery",
      data: data,
