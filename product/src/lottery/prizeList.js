...
function showPrizeList(currentPrizeIndex) {
  ...
  // }
  let htmlCode = `<div class="prize-mess">正在抽取<label id="prizeType" class="prize-shine">${currentPrize.text}</label>
  <label id="prizeText" class="prize-shine">${currentPrize.title}</label>，共
  <label id="prizeSize" class="prize-shine">${currentPrize.count}</label>个，剩余
  <label id="prizeLeft" class="prize-shine">${currentPrize.count}</label>个</div><ul class="prize-list">`;
  prizes.forEach(item => {
...
function resetPrize(currentPrizeIndex) {
  ...
  document.getElementById("lottery").classList.remove("none");
}
...
let setPrizeData = (function () {
  return function (currentPrizeIndex, count, isInit) {
    ...
    if (!prizeElement.prizeType) {
      ...
      prizeElement.prizeSize = document.querySelector("#prizeSize");
    }
    ...
    if (lasetPrizeIndex !== currentPrizeIndex) {
      ...
      prizeElement.prizeSize.textContent = currentPrize.count;
    ...
    if (count <= 0 && currentPrizeIndex === 0) {
      document.getElementById("lottery").classList.add("none");
    } else {
      document.getElementById("lottery").classList.remove("none");
    }
  };
